    ...
    <table class="table table-bordered">
        ...
        <?php foreach ($page->items as $task) { ?>
            ...
            <td><?= $task->tasks->getStatus() ?></td>
                ...
                <?php if ($task->tasks->status == 'Поиск') { ?>
                <td><?= $this->tag->linkTo(['auctions/show/' . $task->tasks->getTaskid(), 'Тендер']) ?></td>
                <?php } elseif ($task->tasks->status == 'Выполняется') { ?>
                <td><?= $this->tag->linkTo(['coordination/index/' . $task->tasks->getTaskid(), 'Чат']) ?></td>
                <?php } ?>
            </tr>
        <?php } ?>
        <?php } ?>
...
<div class="row">
    ...
    <div class="col-sm-11">
        <nav>
            <ul class="pagination">
                <li><?= $this->tag->linkTo(['tasks/workingtasks/' . $userId, 'Первая']) ?></li>
                <li><?= $this->tag->linkTo(['tasks/workingtasks/' . $userId . '?page=' . $page->before, 'Предыдущая']) ?></li>
                <li><?= $this->tag->linkTo(['tasks/workingtasks/' . $userId . '?page=' . $page->next, 'Следующая']) ?></li>
                <li><?= $this->tag->linkTo(['tasks/workingtasks/' . $userId . '?page=' . $page->last, 'Последняя']) ?></li>
            </ul>
        </nav>
    </div>
</div>
